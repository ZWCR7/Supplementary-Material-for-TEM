...
DeLassoTest = function(Y, X, M, A, beta0, L0, S0, deltaL, lambdaL, LambdaMat, HSigma, MB = 1000, alpha = 0.05, tol = 1e-04, iter_max = 200)
{
  ...
  TnArr  = array(0, dim = c(R, C, d))
  E = matrix(rnorm(n*MB), n, MB)
  BootArr = array(0, dim = c(R, C, d, MB))
  ...
  for (r in 1:R)
  {
    for (c in 1:C)
    {
      Errrc = Y[, r, c] - X[, r, c, ] %*% betahat[r, c, ] - M[, r, c]*Lhat[r, c] - A[, r, c, ] %*% Shat[r, c, ]
      
      Thetahatrc = debiasingMatrix(A[, r, c, ], is_wide = T, n, rows = 1:d)
      Stilderc = Shat[r, c, ] + Thetahatrc %*% t(A[, r, c, ]) %*% (Errrc)/n
      
      TnArr[r, c, ] = sqrt(n)*abs(Stilderc)
      
      Sigmahatrc = HSigma[r, c]
      BootArr[r, c, ,  ] = Sigmahatrc*(Thetahatrc %*% t(A[, r, c, ]) %*% E)/sqrt(n)
  ...
  ThresV = apply(BootArr, 4, max)
  Thres = quantile(ThresV, 1 - alpha)
  
  reject = (Tn > Thres)
  
  return(list(Tn = Tn, TnArr = TnArr, Thres = Thres, BootArr = BootArr, reject = reject))
}