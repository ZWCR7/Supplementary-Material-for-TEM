...
for (region in 1:length(Rv))
{
  ...
  for (set in 1:length(StrueList))
  {
    ...
    filename = paste0('Results_Independent/R_', R, 'C_', C, 'Signal', set + 1, '.RData')
    load(filename)
    ...
    save(list = c('RES'), file = paste0('Results_Independent/Post_Estimation_R_', R, 'C_', C, 'Signal', set + 1, '.RData'))
    rm(RES); gc()
  }
...
for (set in 1:5)
{
  filename = paste0('Results_Independent/Post_Estimation_R_12C_12Signal', set + 1, '.RData')
  load(filename)
  ...
  filename = paste0('Results_Independent/Post_Estimation_R_16C_16Signal', set + 1, '.RData')
  load(filename)
...
plot(delta, RMSE_Neighbor16, main = 'Relative RMSE, Independent Design', ylab = '', cex.main = 3.5, cex.lab = 3, cex.axis = 3, lty = 2, col = 'red', type = 'l', lwd = 3, ylim = c(0, 1))
