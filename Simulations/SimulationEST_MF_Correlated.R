...
for (region in 1:length(Rv))
{
  ...
  for (r in 1:R)
  {
    for (c in 1:C)
    {
      ...
      indexv = which((Matdis <= 2) & (Matdis > 0))
      numv = length(indexv)
  ...
  for (set in 1:length(StrueList))
  {
    ...
    filename = paste0('Results_Correlated/MF_R_', R, 'C_', C, 'Signal', set + 1, '.RData')
    load(filename)
    ...
    save(list = c('RES'), file = paste0('Results_Correlated/Post_Estimation_MF_R_', R, 'C_', C, 'Signal', set + 1, '.RData'))
    rm(RES); gc()
  }
...
for (set in 1:5)
{
  filename = paste0('Results_Correlated/Post_Estimation_MF_R_12C_12Signal', set + 1, '.RData')
  load(filename)
  ...
  filename = paste0('Results_Correlated/Post_Estimation_MF_R_16C_16Signal', set + 1, '.RData')
  load(filename)
...
plot(delta, RMSE_Neighbor16, main = 'Relative RMSE, Correlated Design', ylab = '', cex.main = 3.5, cex.lab = 3, cex.axis = 3, lty = 2, col = 'red', type = 'l', lwd = 3, ylim = c(0, 0.5))
...
legend('topright', legend = c('ATE-Post, R=12, C=12', 'ATE-Post, R=16, C=16', 'ATE-MeanField, R=12, C=12', 'ATE-MeanField, R=16, C=16'), 
       lty = c(1, 2, 1, 2), col = c('black', 'black', 'red', 'red'), lwd = 3, cex = 3.5)
dev.off()
